...
interface TripExpenseScreenProps {
  onBack: () => void;
}
...
const TripExpenseScreen = ({ onBack }: TripExpenseScreenProps) => {
  ...
  const handleAddExpense = () => {
    ...
    const newExpense: Expense = {
      ...
      description: expenseDescription || categories.find(c => c.id === selectedCategory)?.label || "Expense",
    ...
    <div className="min-h-screen bg-background flex flex-col max-w-md mx-auto pb-24">
        ...
        <div className="glass-card rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              ...
              <p className={`font-display text-lg font-semibold ${remaining >= 0 ? 'text-success' : 'text-destructive'}`}>
          ...
          <div className="h-3 bg-secondary rounded-full overflow-hidden mb-2">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                spentPercentage > 100 ? 'bg-destructive' : spentPercentage > 80 ? 'bg-warning' : 'bg-primary'
              }`}
      ...
      {expensesByCategory.length > 0 && (
        <div className="px-6 mb-6 animate-slide-up" style={{ animationDelay: "0.1s" }}>
          <h2 className="font-display font-semibold text-foreground mb-3 text-sm">By Category</h2>
          <div className="flex gap-2 overflow-x-auto pb-2 -mx-6 px-6">
            {expensesByCategory.map((cat) => {
              ...
              return (
                <div
                  key={cat.id}
                  className="glass-card rounded-xl p-3 flex-shrink-0 min-w-[100px]"
                >
                  <div className={`w-8 h-8 rounded-lg ${cat.color} flex items-center justify-center mb-2`}>
                    <Icon className="w-4 h-4 text-white" />
                  </div>
                  <p className="text-xs text-muted-foreground">{cat.label}</p>
                  <p className="font-semibold text-foreground text-sm">RM {cat.total.toLocaleString()}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* Add Expense Button */}
      {!showAddForm && (
        <div className="px-6 mb-4 animate-slide-up" style={{ animationDelay: "0.15s" }}>
          <Button
      ...
      {/* Expenses List */}
      <div className="px-6 animate-slide-up" style={{ animationDelay: "0.2s" }}>
        <h2 className="font-display font-semibold text-foreground mb-3 text-sm">Recent Expenses</h2>
        <div className="space-y-2">
          {expenses.map((expense, index) => {
            ...
            return (
              <div
                key={expense.id}
                className="glass-card rounded-xl p-3 flex items-center gap-3 animate-slide-up"
                style={{ animationDelay: `${0.25 + index * 0.05}s` }}
              >
                <div className={`w-10 h-10 rounded-lg ${getCategoryColor(expense.category)} flex items-center justify-center`}>
